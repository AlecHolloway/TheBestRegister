...
def main():
    ...
    layoutMain = [
                  ...
                  [sg.Button('Remove'), sg.Button('Admin login'), sg.Button('EXIT')],
    ...
    while True:
        ...
        if eventMain in ('Scan'):
            ...
            if receiptList != []:
                windowMain.Element('_DISPLAY_').Update(stringOutput(receiptList))
            else:
        ...
        while windowPayActive:
            ...
            if eventPay in ('Cash', 'Check', 'Credit', 'Debit'):
                ...
                ##convert item list to a string to pass into the database
                string_list = ','
                item = string_list.join(itemList)

                ##adding receiptList to database
                database.update_database(transactionCount ,item , priceSum, payMethod)
